...
public class AlarmPlugin : MonoBehaviour {
	...
	public void SetOneTimeAlarm(int hour, int minute,string alarmTile,string alarmMessage,string alarmTickerMessage){
		#if UNITY_ANDROID
		if(Application.platform == RuntimePlatform.Android){
			jo.CallStatic("setOneTimeAlarm",hour,minute,alarmTile,alarmMessage,alarmTickerMessage);
		}else{
			Utils.Message(TAG,"warning: must run in actual android device");
		}
		#endif
	...
	/// <param name="alarmMessage">Alarm message.</param>
	/// <param name="alarmTickerMessage">Alarm ticker message.</param>
	public void SetRepeatingAlarm(int hour, int minute,string alarmTile,string alarmMessage,string alarmTickerMessage){
		#if UNITY_ANDROID
		if(Application.platform == RuntimePlatform.Android){
			jo.CallStatic("setRepeatingAlarm",hour,minute,alarmTile,alarmMessage,alarmTickerMessage);
		}else{
			Utils.Message(TAG,"warning: must run in actual android device");
		}
		#endif
	...
	/// <param name="alarmMessage">Alarm message.</param>
	/// <param name="alarmTickerMessage">Alarm ticker message.</param>
	public void SetRepeatingAlarmWithInterval(int hour, int minute,int repeatDelay,string alarmTile,string alarmMessage,string alarmTickerMessage){
		#if UNITY_ANDROID
		if(Application.platform == RuntimePlatform.Android){
			jo.CallStatic("setRepeatingAlarmWithInterval",hour,minute,repeatDelay,alarmTile,alarmMessage,alarmTickerMessage);
		}else{
			Utils.Message(TAG,"warning: must run in actual android device");
		}
		#endif
